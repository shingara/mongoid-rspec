...
module Mongoid
  module Matchers
    module Associations
      ...
      #HAS_MANY_AS_ARRAY = Mongoid::Relations::Referenced::ManyAsArray
      ...
      class HaveAssociationMatcher
        ...
        ##
        # No more exist on Mongoid 2.0.0.rc.1
        def stored_as(store_as)
          puts "There are no more stored_as on Mongoid"
        ...
        def matches?(actual)
          ...
          association = @actual.relations[@association[:name]]
          ...
          if association.relation != @association[:type]
            @negative_result_message = "#{@actual.inspect} #{type_description(association.relation, false)} #{@association[:name]}"
            return false
          else
            @positive_result_message = "#{@actual.inspect} #{type_description(association.relation, false)} #{@association[:name]}"
          end
          ...
          if @association[:inverse_of]
            if @association[:inverse_of].to_s != association.inverse.to_s
              @negative_result_message = "#{@positive_result_message} which is an inverse of #{association.inverse}"
              return false
            else
              @positive_result_message = "#{@positive_result_message} which is an inverse of #{association.inverse}"
            end
          end
